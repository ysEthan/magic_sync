        ...
        with connection.cursor() as cursor:
            for product in products:
                try:
                    ...
                    # 检查并更新申报英文名称
                    if not product.get('declareNameEn'):
                        logger.info(f"商品 {product['goodsNo']} 缺少英文申报名称，开始更新...")
                        update_result = update_declareName(product)
                        if update_result:
                            logger.info(f"商品 {product['goodsNo']} 英文申报名称更新成功")
                        else:
                            logger.warning(f"商品 {product['goodsNo']} 英文申报名称更新失败")
