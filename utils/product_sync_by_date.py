...
class ProductSyncByDate(ProductSyncBase):
    ...
    def sync_products(self, start_time=None, end_time=None, page=1):
        ...
        body = {
            ...
            # "end_time": end_time
            "start_time": "2025-03-01 16:53:33",
...
def sync_products_by_date(request):
    """定时任务调用的同步函数 - 默认同步最近24小时数据"""
    try:
        sync = ProductSyncByDate()
        ...
        return JsonResponse({
            'status': 'success',
            'message': f'商品数据同步成功，共处理 {processed_count} 条数据'
        })
    except Exception as e:
...
def sync_products_incremental(request):
    """增量同步函数 - 只同步最近1小时的数据"""
    try:
        sync = ProductSyncByDate()
        start_time = datetime.now()- timedelta(hours=240*45)
        end_time = datetime.now()
        
        logger.info(f"开始执行增量同步任务: {start_time} 到 {end_time}")
        api_data = sync.sync_products(
            start_time=start_time.strftime('%Y-%m-%d %H:%M:%S'),
            end_time=end_time.strftime('%Y-%m-%d %H:%M:%S')
        )
        processed_count = sync.process_products(api_data['data'])
        logger.info(f"增量同步任务完成，共处理 {processed_count} 条数据")
        return JsonResponse({
            'status': 'success',
            'message': f'增量同步成功，共处理 {processed_count} 条数据'
        })
    except Exception as e:
        logger.error(f"增量同步任务失败: {str(e)}")
        return JsonResponse({
            'status': 'error',
